...
class HighwayEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.obstacles.append(self.ego)
        self.manager.add(self.ego)
    ...
    def step(self, action):
        ...
        # --------------------- Decision Tree ------------------------
            ...
            if obstacle.signtime == self.time_step: # Time to change lane
                ...
                obs_ahead = min([obs for obs in self.manager.holding_system[obstacle.lane] if obs.position > obstacle.position], key=lambda obs: obs.position - obstacle.position)
                if obs_ahead is not None and obs_ahead.signtime == obstacle.signtime and obs_ahead.signtime > 0:
                    if obstacle.speed < obstacle.target_speed:
                        ...
                        obstacle.signtime = -1
                        obstacle.DTact(1)
                        if obstacle == self.ego:
                            self.ego.action_viz =3
                    elif obstacle.speed > obstacle.target_speed:
                        obstacle.sign = 0
                        obstacle.signtime = -1
                        obstacle.DTact(-1)   
                        if obstacle == self.ego:
                            self.ego.action_viz =4
            ...
            # No obstacles ahead or obstacle ahead is faster
            if obstacle.speed < obstacle.target_speed:
                obstacle.sign = 0
                obstacle.signtime = -1
                if obstacle == self.ego:
    ...
    def render(self, mode='human'):
        if mode == 'human':
            ...
            action_dict = {'None': 'None', 0: 'maintain',1: 'changeLaneR',2: 'changeLaneL',3: 'accelerate',
                   4: 'decelerate'}
...
if __name__=='__main__':
    ...
    for i in range(timestep):
        _, reward, _, _ = env.step('dummy')
        env.render()
        # plt.show(block=False)
    print(f"Out-of-boundary rate: {env.outOfBoundary_time/env.num_scenario:.2f}")
    print(f"Collision rate: {env.collision_time/env.num_scenario:.2f}")
    print(f"Lane-changing rate: {env.lane_change_time/env.num_scenario:.2f}")
    print(f"Target velocity keeping rate: {env.v_keeping/env.time_step}")
